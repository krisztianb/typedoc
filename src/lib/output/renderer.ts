...
> {
    ...
    get theme(): Theme {
        ...
        throw new Error("No theme is set");
    }
    ...
    private renderDocument(page: PageEvent): boolean {
        ...
        page.template ??= this.theme.resources.templates
            .getResource(page.templateName)
            ?.getTemplate();

        if (!page.template) {
            throw new Error("No template is set");
        }
